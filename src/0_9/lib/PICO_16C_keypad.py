...
# Enter
KP_ENTER = 34
KP_FMACRO = KP_ENTER + 40
KP_LST_X = KP_ENTER + 80

KP_ENTER2 = 24
KP_WINDOW = KP_ENTER2 + 40
KP_GMACRO = KP_ENTER2 + 80
...
key_functions = [ 'f_mathOp', 'f_digit', 'f_digit', 'f_digit', 'f_digit', 'f_digit', 'f_digit', 'f_digit', 'f_digit', 'f_digit',
 ...
 # shift f:                 
                  ...
                  'f_mathOp', 'f_compl', 'f_compl', 'f_compl', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef',
                  'f_mathOp', 'f_digit', 'f_stat',  'f_undef', 'f_fmacr', 'f_float',  'f_undef', 'f_shift', 'f_shift', 'f_splsh',
 # shift g:                 
                  'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef',
                  'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_undef',
                  'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_gmacr', 'f_CLx',   'f_undef', 'f_rollu', 'f_undef', 'f_undef',
                  'f_undef', 'f_undef', 'f_undef', 'f_undef', 'f_LastX', 'f_pan_d', 'f_pan_d', 'f_shift', 'f_shift', 'f_splsh']
...
float_num_keys = (KP_DECIMAL_PT, KP_ZERO, KP_ONE, KP_TWO, KP_THREE, KP_FOUR, KP_FIVE,
                          KP_SIX, KP_SEVEN, KP_EIGHT, KP_NINE, KP_EEX)
all_num_keys = (KP_DECIMAL_PT, KP_ZERO, KP_ONE, KP_TWO, KP_THREE, KP_FOUR, KP_FIVE,
                    KP_SIX, KP_SEVEN, KP_EIGHT, KP_NINE,
                    KP_A, KP_B, KP_C, KP_D, KP_E, KP_F)
operator_keys = (KP_ADD, KP_SUBTRACT, KP_MULTIPLY, KP_DIVIDE)  # +-x/
base_keys = (KP_BIN, KP_OCT, KP_DEC, KP_HEX, KP_FLOAT)
show_base_conversion_keys = (KP_SHOW_BIN, KP_SHOW_OCT, KP_SHOW_DEC, KP_SHOW_HEX)
window_value_keys = (KP_ZERO, KP_ONE, KP_TWO, KP_THREE, KP_FOUR, KP_FIVE, KP_SIX, KP_SEVEN)
...
class Calculator():
    ...
    def __init__(self):
        ...
        self.km = keypad.KeyMatrix( 
                    row_pins=(board.GP9, board.GP8, board.GP7, board.GP6, board.GP27),   # after protoboard fab
                    column_pins=(board.GP2, board.GP3, board.GP4, board.GP5,
        ...
        self.decimals_entered = 0
        self.complement = 2
        self.entry_in_progress = False
        # self.flagIndicators = 0
        self.CarryBit = False
        self.OutOfRange = False
        self.ExponentMode = False
        self.exponents_entered = False
        ...
        # self.timedOut = False
        self.macroState = MACRO_STATE_OFF
        return
    
    def check_keypress(self): # non-blocking
        self.key = -1
        event = self.km.events.get()
        if event:
            if event.pressed:
                self.key = event.key_number
                if self.shift == SHIFT_STATE_F:
                    self.key += 40
                elif self.shift == SHIFT_STATE_G:
                    self.key += 80
                # self.timedOut = False
                self.timeOut = time.time() + TIMEOUT_INTERVAL
        return
    ...
    def wait_keypress_w_timeout(self):
    ...
    def f_fmacr(self, key):
        self.shift = SHIFT_STATE_OFF
        print("\nF macro")
        return
    
    def f_gmacr(self, key):
        self.shift = SHIFT_STATE_OFF
        print("\nG macro")
        self.macroState = MACRO_STATE_1
        return
    ...
    def f_compl(self, key):
        if self.key == KP_UNSGN:
            self.complement = 0
        elif self.key == KP_ONES_COMP:
            self.complement = 1
        else: # KP_TWOS_COMP
            self.complement = 2
        self.shift = SHIFT_STATE_OFF
    ...
    def f_digit(self, key):
        ...
        if self.shift == SHIFT_STATE_FLOAT:			# next number is number of decimal places
            if key in float_num_keys:
                if key == KP_DECIMAL_PT:
                    self.ExponentMode = True
                    self.decimalPlaces = 6				# 6 is standard for the HP 16C
                else:
                    self.ExponentMode = False
                    self.decimalPlaces = int(keys_char[key])
                self.base = BASE_STATE_FLOAT
                self.Xreg = float(self.Xreg)
            self.shift = SHIFT_STATE_OFF
        else:
            ...
            elif self.base == BASE_STATE_FLOAT and key in float_num_keys:
                if not self.entry_in_progress:
                    self.Xbuff = "0"
                    self.Yreg = self.Xreg
                    self.Xreg = 0.0
                self.entry_in_progress = True
                if not self.ExponentEntry:
                    if key in dec_num_keys:
                        self.Xbuff = self.Xbuff + keys_char[key]
                        if self.decimal_entered:
                            self.decimals_entered += 1
                    elif key == KP_DECIMAL_PT:
                        if not self.decimal_entered:    # ignore otherwise
                        ...
                        if not self.ExponentEntry:
                            ...
                            self.Xbuff = self.Xbuff + "e0"
                            self.exponents_entered = False;
                else: #exponent entry
                    if not self.exponents_entered and key in dec_num_keys:
                        self.exponents_entered = True
                        self.Xbuff = self.Xbuff[:-1]  #remove the zero we added earlier
                    self.Xbuff = self.Xbuff + keys_char[key]
                self.Xreg = float(self.Xbuff)
    ...
    def f_float(self, key):
        self.shift = SHIFT_STATE_FLOAT     # expecting 0..9, or . next
        return
    ...
    def f_ENTER(self, key):
        ...
        self.ExponentEntry = False
        self.strindex = 0
        if self.base == BASE_STATE_FLOAT:
            self.Treg = float(self.Zreg)
            self.Zreg = float(self.Yreg)
            self.Yreg = float(self.Xreg)
        else:
            self.Treg = self.Zreg
            self.Zreg = self.Yreg
            self.Yreg = self.Xreg
